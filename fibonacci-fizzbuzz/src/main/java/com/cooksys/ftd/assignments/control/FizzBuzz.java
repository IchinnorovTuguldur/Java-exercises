...
public class FizzBuzz {
    ...
    public static String[] messages(int start, int end) throws IllegalArgumentException {
        ...
        for(int i=start; i<end; i++) {
        	...
        	if(currentMessage != null) {
        		String temp[] = new String[answers.length + 1];
        		for(int j=0; j < answers.length; j++) {
        			temp[j] = answers[j];
        		}
        		temp[temp.length - 1] = currentMessage;
        		answers = temp;
        	}
        }
    ...
    public static void main(String[] args) {
    	...
    	String[] finalnAnswer = messages(3,5); //save this somehow, print the values;
        
    	for(int k=0; k<finalnAnswer.length; k++) {
        	System.out.println(finalnAnswer[k]);
        }
    	System.out.println(finalnAnswer.length);
    }
